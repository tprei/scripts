...
import { makeMockChatProvider, makeMockFileHandler } from "./test-helpers.js"
...
describe("Observer", () => {
  ...
  describe("onSessionStart", () => {
    it("sends a session start message for task mode", async () => {
      const chat = makeMockChatProvider()
      const observer = new Observer(chat, 3000, { textFlushDebounceMs: 1500, activityEditDebounceMs: 2000 })
      const meta = makeMeta({ mode: "task" })

      await observer.onSessionStart(meta, "fix the bug")

      expect(chat.sendMessage).toHaveBeenCalledOnce()
      const msg = (chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[0][0]
  ...
  describe("onEvent — tool requests", () => {
    ...
    it("sends new activity message after throttle window expires", async () => {
      const chat = makeMockChatProvider()
      const observer = new Observer(chat, 1000, { textFlushDebounceMs: 1500, activityEditDebounceMs: 2000 })
      const meta = makeMeta()
    ...
    it("flushes text buffer before tool request", async () => {
      ...
      expect(chat.sendMessage).toHaveBeenCalledTimes(2)
      expect((chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[0][0]).toContain("Reply")
      expect((chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[1][0]).toContain("Activity")
    })
  ...
  describe("onSessionComplete", () => {
    ...
    it("flushes remaining text buffer before completing", async () => {
      ...
      expect(chat.sendMessage).toHaveBeenCalledTimes(2)
      expect((chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[0][0]).toContain("Reply")
      expect((chat.sendMessage as ReturnType<typeof vi.fn>).mock.calls[1][0]).toContain("Complete")
    })
  })
  ...
  describe("screenshot detection", () => {
    ...
    it("sends screenshot photo after detecting browser_take_screenshot tool", async () => {
      const chat = makeMockChatProvider()
      const files = makeMockFileHandler()
      const observer = new Observer(chat, 3000, { textFlushDebounceMs: 1500, activityEditDebounceMs: 2000 }, files)
      ...
      expect(files.sendPhoto).toHaveBeenCalledOnce()
      expect(files.sendPhoto).toHaveBeenCalledWith(
        path.join(screenshotDir, "screenshot-1.png"),
        "42",
        "\u{1F4F8} screenshot-1.png",
      )
    })
    ...
    it("sends pending screenshots on session complete", async () => {
      ...
      expect(files.sendPhoto).toHaveBeenCalledWith(
        ...
        "42",
        "\u{1F4F8} page-capture.png",
      )
    })

    it("skips screenshots gracefully when no FileHandler is provided", async () => {
      const chat = makeMockChatProvider()
      const observer = new Observer(chat, 3000, { textFlushDebounceMs: 1500, activityEditDebounceMs: 2000 })
      const meta = makeMeta({ cwd: tmpDir })
      ...
      // No crash, no file operations attempted
      expect(chat.sendMessage).toHaveBeenCalledOnce()
    })

    it("skips base64 image tool responses when no FileHandler is provided", async () => {
      const chat = makeMockChatProvider()
      const observer = new Observer(chat, 3000, { textFlushDebounceMs: 1500, activityEditDebounceMs: 2000 })
      const meta = makeMeta()
      ...
      await observer.onEvent(meta, {
        ...
        message: {
          ...
          content: [{
            type: "toolResponse",
            id: "t1",
            toolResult: [{ type: "image", data: "aGVsbG8=", mimeType: "image/png" }],
          }],
        },
      })

      // No crash — just silently skipped
    })
  })
  ...
  describe("threadId conversion", () => {
    it("converts numeric threadId to string when calling ChatProvider", async () => {
      const chat = makeMockChatProvider()
      const observer = new Observer(chat, 3000, { textFlushDebounceMs: 1500, activityEditDebounceMs: 2000 })
      const meta = makeMeta({ threadId: 99 })

      await observer.onSessionStart(meta, "task")

      expect(chat.sendMessage).toHaveBeenCalledWith(expect.any(String), "99")
    })
  })
})
