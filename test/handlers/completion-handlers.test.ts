...
import { makeMockChatProvider, makeMockObserver, makeMockStats } from "../test-helpers.js"
...
describe("ShipAdvanceHandler", () => {
  function makeShipDeps() {
    return {
      chat: makeMockChatProvider({ sendMessage: vi.fn().mockResolvedValue({ ok: true, messageId: "1" }) }),
  ...
  it("skips non-ship modes", async () => {
    ...
    const handler = new ShipAdvanceHandler(
      deps.chat, deps.observer, deps.shipPipeline,
  ...
  it("sends paused message on error state", async () => {
    ...
    expect(ctx.handled).toBe(true)
    expect(deps.chat.sendMessage).toHaveBeenCalledWith(
...
describe("ModeCompletionHandler", () => {
  ...
  it("handles think mode completion", async () => {
    ...
    const handler = new ModeCompletionHandler(deps.chat, deps.observer, deps.pinnedMessages)
    ...
    expect(deps.chat.sendMessage).toHaveBeenCalledWith("think done: test-slug", ts.threadId)
  })
  ...
  it("handles review mode completion", async () => {
    ...
    expect(ctx.handled).toBe(true)
    expect(deps.chat.sendMessage).toHaveBeenCalledWith("review done: test-slug", ts.threadId)
  })
  ...
  it("handles plan mode completion", async () => {
    ...
    expect(ctx.handled).toBe(true)
    expect(deps.chat.sendMessage).toHaveBeenCalledWith("plan done: test-slug", ts.threadId)
  })
...
describe("QualityGateHandler", () => {
  it("runs quality gates for completed sessions", async () => {
    ...
    const chat = makeMockChatProvider({ sendMessage: vi.fn().mockResolvedValue({ ok: true, messageId: "1" }) })
    const pusher = { pushToConversation: vi.fn() }
    const handler = new QualityGateHandler(chat, pusher)
    const ctx = makeCtx()

    await handler.handle(ctx)

    expect(ctx.qualityReport).toEqual({ results: [{ gate: "tsc", passed: true, output: "ok" }], allPassed: true })
    expect(chat.sendMessage).toHaveBeenCalledWith("quality report", 100)
  })
  ...
  it("skips for non-completed state", async () => {
    const chat = makeMockChatProvider({ sendMessage: vi.fn() })
    const pusher = { pushToConversation: vi.fn() }
    const handler = new QualityGateHandler(chat, pusher)
    const ctx = makeCtx({ state: "errored" })
...
describe("TaskCompletionHandler", () => {
  ...
  it("orchestrates flush, inner handlers, and cleanup", async () => {
    ...
    expect(deps.chat.sendMessage).toHaveBeenCalledWith("task done: test-slug", 100)
