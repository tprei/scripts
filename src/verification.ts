...
export async function checkMergeConflicts(prUrl: string, cwd: string): Promise<MergeConflictResult> {
  ...
  if (state === "UNKNOWN") {
    // GitHub may need time to compute mergeability — retry once after a short wait
    await sleep(5_000)
...
/**
 * Build a prompt for a completeness review agent that checks whether
 * a DAG node's implementation matches its specification.
 *
 * The caller spawns a review session with this prompt. The session must
 * output "VERIFICATION PASSED" if the code is complete, or describe
 * specific deficiencies if not.
 */
